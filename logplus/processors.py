...
class JSONRenderer:
    ...
    Args:
        dumps_kw:
            ...
            will disable support for ``__logplus__``-based serialization.
    ...
    .. versionadded:: 0.2.0 Support for ``__logplus__`` serialization method.
...
def _json_fallback_handler(obj: Any) -> Any:
    """
    Serialize custom datatypes and pass the rest to __logplus__ & repr().
    """
    # circular imports :(
    from logplus.threadlocal import _ThreadLocalDictWrapper

    if isinstance(obj, _ThreadLocalDictWrapper):
        return obj._dict

    try:
        return obj.__logplus__()
...
class ExceptionRenderer:
    ...
    *exception_formatter* that is passed:
    ...
    - If you pass a :class:`~logplus.tracebacks.ExceptionDictTransformer`, it
    ...
    .. seealso::
        :doc:`exceptions` for a broader explanation of *logplus*'s exception
        features.
...
:class:`~logplus.tracebacks.ExceptionDictTransformer`.
...
class StackInfoRenderer:
    ...
    Args:
        additional_ignores:
            By default, stack frames coming from *logplus* are ignored. With
            ...
            found that doesn't start with *logplus* or one of
...
class CallsiteParameter(enum.Enum):
    ...
    `logplus.processors.CallsiteParameterAdder` processor class.
...
class CallsiteParameterAdder:
    ...
    not originate from *logplus* then the callsite information will be
    ...
    determined from the stack trace, ignoring all intra-logplus calls, calls
    ...
    .. note::
        ...
        When used with `logplus.stdlib.ProcessorFormatter` the most efficient
        configuration is to either use this processor in ``foreign_pre_chain``
        of `logplus.stdlib.ProcessorFormatter` and in ``processors`` of
        `logplus.configure`, or to use it in ``processors`` of
        `logplus.stdlib.ProcessorFormatter` without using it in
        ``processors`` of `logplus.configure` and ``foreign_pre_chain`` of
        `logplus.stdlib.ProcessorFormatter`.
    ...
    def __init__(
        ...
        # module should not be logging using logplus.
    ...
    def __call__(
        ...
        from_logplus: bool | None = event_dict.get("_from_logplus")
        # If the event dictionary has a record, but it comes from logplus,
        # then the callsite parameters of the record will not be correct.
        if record is not None and not from_logplus:
