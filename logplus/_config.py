...
- `getting-started`.
- logplus.stdlib.recreate_defaults()'s docstring.
"""
...
def is_configured() -> bool:
    """
    Return whether *logplus* has been configured.

    If `False`, *logplus* is running with builtin defaults.
...
def get_config() -> dict[str, Any]:
    ...
    .. note::
       ...
       Changes to the returned dictionary do *not* affect *logplus*.
...
def get_logger(*args: Any, **initial_values: Any) -> Any:
    ...
    >>> from logplus import get_logger
    >>> log = get_logger(y=23)
    >>> log.info("hello", x=42)
    ...
    Returns:
        ...
        and is `logplus.BoundLogger` by default.
...
"""
CamelCase alias for `logplus.get_logger`.
...
def configure(
    ...
    Args:
        ...
        wrapper_class:
            ...
            `logplus.BoundLogger`.  See `standard-library`, :doc:`twisted`,
            and `custom-wrappers`.
...
def configure_once(
    ...
    """
    Configures if logplus isn't configured yet.
