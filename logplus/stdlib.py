...
See also :doc:`logplus's standard library support <standard-library>`.
"""
...
class _FixedFindCallerLogger(logging.Logger):
    ...
    *logplus*'s extra frames.
    ...
    def findCaller(
        ...
        """
        Finds the first caller frame outside of logplus so that the caller
...
class BoundLogger(BoundLoggerBase):
    """
    Python Standard Library version of `logplus.BoundLogger`.
    ...
    Use it like::
        ...
        logplus.configure(
            wrapper_class=logplus.stdlib.BoundLogger,
        )
    ...
    def new(self, **new_values: Any) -> BoundLogger:
        ...
        those wrapped by `logplus.threadlocal.wrap_dict` when threads
        are re-used.
        """
...
def get_logger(*args: Any, **initial_values: Any) -> BoundLogger:
    """
    Only calls `logplus.get_logger`, but has the correct type hints.

    .. warning::

       Does **not** check whether -- or ensure that -- you've configured
       *logplus* for standard library :mod:`logging`!
...
class AsyncBoundLogger:
    ...
    # not and we need the class in `logplus.configure()`.
...
class LoggerFactory:
    ...
    >>> from logplus import configure
    >>> from logplus.stdlib import LoggerFactory
    ...
    def __call__(self, *args: Any) -> logging.Logger:
        ...
        :func:`logplus.get_logger` call.  For example
        ``logplus.get_logger("foo")`` would cause this method to be called
        ...
        # We skip all frames that originate from within logplus or one of the
        # configured names.
...
class PositionalArgumentsFormatter:
    ...
    ``positional_args`` is populated by `logplus.stdlib.BoundLogger` or
    can be set manually.
...
def filter_by_level(
    ...
    >>> import logging
    >>> from logplus.stdlib import filter_by_level
...
def add_log_level_number(
    ...
    ``logplus.stdlib._log_levels._NAME_TO_LEVEL``.
    ...
    *logplus*'s processor chain.
    ...
    Args:
        foreign_pre_chain:
            ...
            **non**-*logplus* log entries before the event dictionary is
            passed to *processors*. (default: `None`)

        processors:
            A chain of *logplus* processors that is used to process **all**
            log entries. The last one must render to a `str` which then gets
            passed on to `logging` for output.

            Compared to *logplus*'s regular processor chains, there's a few
            differences:

            - The event dictionary contains two additional keys:

              #. ``_record``: a `logging.LogRecord` that either was created
                  using `logging` APIs, **or** is a wrapped *logplus* log
                  entry created by `wrap_for_formatter`.

              #. ``_from_logplus``: a `bool` that indicates whether or not
                 ``_record`` was created by a *logplus* logger.

              Since you most likely don't want ``_record`` and
              ``_from_logplus`` in your log files,  we've added the static
            ...
            - Since this is a `logging` *formatter*, raising
              `logplus.DropEvent` will crash your application.
        ...
        logger:
            Logger which we want to push through the *logplus* processor
        ...
        processor:
            A single *logplus* processor used for rendering the event
    ...
    def format(self, record: logging.LogRecord) -> str:
        """
        Extract *logplus*'s `event_dict` from ``record.msg`` and format it.
        ...
        *record* has been patched by `wrap_for_formatter` first though, so the
        ...
        meth_name = getattr(record, "_name", "__logplus_sentinel__")

        ed: ProcessorReturnValue
        if logger is not _SENTINEL and meth_name != "__logplus_sentinel__":
            ...
            # Non-logplus allows to run through a chain to prepare it for the
    ...
    def remove_processors_meta(
        ...
        """
        Remove ``_record`` and ``_from_logplus`` from *event_dict*.
