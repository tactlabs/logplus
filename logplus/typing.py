...
"""
Type information used throughout *logplus*.
...
*logplus* makes *no* assumptions about it.
...
Used by `logplus.dev.ConsoleRenderer`.
...
class ExceptionTransformer(Protocol):
    ...
    **Protocol:** A callable that transforms an `ExcInfo` into another
    ...
    Used by `logplus.processors.format_exc_info()` and
    `logplus.processors.ExceptionPrettyPrinter`.
