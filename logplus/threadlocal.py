...
def _deprecated() -> None:
    ...
    if callsite == "logplus.threadlocal":
        return
    ...
    warnings.warn(
        "`logplus.threadlocal` is deprecated, please use "
        "`logplus.contextvars` instead.",
        DeprecationWarning,
...
def as_immutable(logger: TLLogger) -> TLLogger:
    ...
    Args:
        logger (logplus.typing.BindableLogger):
            A logger with *possibly* thread local state.

    Returns:
        :class:`~logplus.BoundLogger` with an immutable context.
...
def tmp_bind(
    ...
    Only works with `logplus.threadlocal.wrap_dict`-based contexts.
    Use :func:`~logplus.threadlocal.bound_threadlocal` for new code.
...
class _ThreadLocalDictWrapper:
    ...
    :func:`logplus.BoundLogger.new` to clear the context.
    """
    ...
    # Proxy methods necessary for logplus.
